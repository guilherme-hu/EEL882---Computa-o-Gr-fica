...
function drawArwing(activeLives = 4) {
    ...
    function tri(v1, v2, v3, col) {
        // Calcula a normal para que a iluminação 3D funcione
    ...
    tri(nose, midBot, midL, cWhite); // Substituindo cShadow por cWhite para luz real
    ...
    // Parede de fundo do motor
    quad3(tailTop, tailL, tailBot, tailR, cDarkEngine);
    
    // Fogo do Motor Principal (Triângulo Interno)
    // Esse aqui será o motor "estético" principal da nave, sempre aceso
    ...
    // Vidro do cockpit (Sobreposto um pouco menor)
    ...
    // 4) Asas Principais
    // Vértices da Asa Esquerda
    let wL1 = v(-25, 0, 10);    // Raiz frente
    let wL2 = v(-110, 0, -40);  // Ponta externa frente
    let wL3 = v(-130, 0, -80);  // Ponta externa trás
    let wL4 = v(-70, 0, -50);   // Encaixe interno
    let wL5 = v(-80, 0, -90);   // Ponta interna trás
    let wL6 = v(-15, 0, -60);   // Raiz trás

    // Vértices da Asa Direita
    let wR1 = v(25, 0, 10);    
    let wR2 = v(110, 0, -40);  
    let wR3 = v(130, 0, -80);  
    let wR4 = v(70, 0, -50);   
    ...
    // Fundo Asa Esquerda
    push(); translate(0, 1, 0); // Desloca 1px para baixo para ser a barriga
    tri(wL1, wL4, wL2, cWhite);
    tri(wL2, wL4, wL3, cWhite);
    quad3(wL4, wL1, wL6, wL5, cWhite);
    pop();

    // Desenhar Asa Direita
    ...
    // 5) Turbinas em baixo das asas    
    // Função para desenhar cada Turbina
    // Depende do número de vidas ativas para decidir quais estão acesas
    function drawJetEngine(cx, cy, cz, active) {
        ...
        // Emissão de Efeito de Fogo -> Rastro de Partículas (Mach Diamonds)
        if (active) {
            push();
            ...
            for (let i = 0; i < 4; i++) {
                ...
                if (t < 6) {
                    ...
                    // Mais distante, fica laranja escuro
    ...
    // 6) Aletas Verticais Azuis 
    // Aleta Esquerda (Superior)
    let fL_TopBaseF = v(-40, 0, 0);
    let fL_TopBaseB = v(-60, 0, -40);
    let fL_TopTip   = v(-85, -60, 20); // Ponta alta, pra fora e pra frente!
    
    tri(fL_TopBaseF, fL_TopTip, fL_TopBaseB, cBlue); // Usando a mesma cor azul, a luz que dá o contraste
    tri(fL_TopBaseF, fL_TopBaseB, fL_TopTip, cBlue);

    // Aleta Esquerda (Inferior)
    let fL_BotTip   = v(-65, 30, -50); // Ponta baixa, pra trás
    tri(fL_TopBaseF, fL_BotTip, fL_TopBaseB, cBlue);
    tri(fL_TopBaseF, fL_TopBaseB, fL_BotTip, cBlue);

    // Aleta Direita (Superior)
